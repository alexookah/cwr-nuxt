      ...
      <div class="flex flex-col gap-2.5">
        <div v-for="song in album.songs" :key="song.name" class="bg-gray-200 p-2.5 rounded-lg">
          ...
          <div class="flex gap-2">
            <a v-for="link in song.links" :key="link.source" :href="link.link" target="_blank" class="bg-gray-300 px-2.5 py-1 rounded text-gray-800">
              <!-- Replace with icons for each source -->
              {{ link.source }}
            </a>
          </div>
        </div>
